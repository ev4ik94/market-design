...
exports.editCost = async (req, res) => {
    try{
        ...
        await Cost.update({cost:data.cost, discount:data.discount},{where:{product_id:req.query.id, type:data.type}}).then(result=>{
            res.status(200).json({success:true, message: 'Cost is updated successfuly'});
        }).catch(err=>{
            res.status(400).json({success:true, message: erre.message});
        })
...
exports.deleteCost = async (req, res) => {
    try{
        ...
        await Cost.destroy({where:{product_id:req.query.id, type:data.type}}).then(result=>{
